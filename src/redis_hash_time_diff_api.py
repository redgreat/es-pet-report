...
# 创建FastAPI应用
app = FastAPI(
    title="Redis Hash时间差异分析API",
    description="分析Redis中HASH格式数据的时间字段之间的差异",
    version="1.0.0"
)
...
class RedisRequest(BaseModel):
    ...
    process_details: Optional[bool] = True  # 是否处理details字段中的JSON数据
...
def calculate_time_differences(r: redis.Redis, pattern: str, count: int = 0, process_details: bool = True) -> Dict[str, Any]:
    """计算Redis中HASH格式数据的时间字段之间的时差"""
    # 记录统计开始时间
    ...
    for key in r.scan_iter(match=pattern):
        ...
        if len(batch_keys) >= batch_size:
            ...
            processed, details_count = process_batch(r, batch_keys, max_diffs, total_diffs, diff_distributions, time_info, process_details)
            ...
            batch_count += 1
            print(f"批次 {batch_count}: 处理耗时 {batch_time:.2f}秒，键数量: {len(batch_keys)}，已处理: {processed_docs}，details文档: {processed_details}，总数: {total_keys}")
...
def process_batch(r: redis.Redis, keys: List[str], max_diffs: Dict, total_diffs: Dict, diff_distributions: Dict, time_info: Dict, process_details: bool) -> tuple:
    """处理一批Redis HASH键
    ...
    for i, hash_data in enumerate(hash_values):
        ...
        try:
            ...
            if process_details and "details" in hash_data:
                details_docs = process_details_field(hash_data["details"], doc_id, max_diffs, total_diffs, diff_distributions, time_info)
                details_count += details_docs

        except Exception as e:
            print(f"处理Hash出错: {e}, key: {keys[i]}")

    return processed_count, details_count
...
async def root():
    """API根路径"""
    return {"message": "Redis Hash时间差异分析API服务正在运行"}
...
async def analyze_time_diffs(request: RedisRequest):
    ...
    try:
        ...
        # 计算时间差异
        results = calculate_time_differences(r, request.pattern, request.count, request.process_details)
...
async def get_text_report(pattern: Optional[str] = "*", count: Optional[int] = 0, process_details: Optional[bool] = True):
    ...
    try:
        ...
        # 计算时间差异
        results = calculate_time_differences(r, pattern, count, process_details)
...
if __name__ == "__main__":
    uvicorn.run("redis_hash_time_diff_api:app", host="0.0.0.0", port=8002, reload=True)
